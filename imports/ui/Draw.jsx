...
class Draw extends Component {
    ...
    InitView(){
            ...
            function __init_controls(selection) {
                selection.each(function() {
                    ...
                    controls.append("path")
                        ...
                        .attr("stroke", "none")
                       // .attr("marker-end","url(.ui-show-hide)")
                        .style("opacity", 0.5);
       ...
       //引入机场图片和添加虚拟层前的初始化
        ...
        // 负责初始图层绘制
            ...
            //.addLayer(overlays);   //虚拟层 使用时打开
        //map.addLayer(overlays);
        d3.json("/data", function(data) { //虚拟层加载json数据
    ...
    handleAddLayer(macAddress, event) {
        ...
        if (!document.getElementById("path-" + macAddress)) {
            ...
            let points = this.getData(macAddress);//从数据库查询旅客轨迹数据
            let pathData = [{"id": "flt-2", "classes": "planned","title": "测试", "points": []}];

            let pathPoint = [];

            for (let i = points.tracks.length - 1; i >= 0 ; --i) {
                pathPoint.push(this.convertLocation(points.tracks[i].point));//将旅客经过的点转成屏幕像素坐标存入pathPoint中
            }

            pathData[0]["points"] = pathPoint;//pathPoint对应旅客轨迹的像素点
    ...
    // 改变轨迹颜色
    handleChangeTrackColor(macAddress, color) {
        d3.select("#path-" + macAddress).attr("stroke", color);
    }
    ...
    handleRemoveAllUserLayer() {
        ...
        console.log("remove-path: All user tracks finished");
    }
            ...
            function monitor(g) {
                g.each(function(data){
                    ...
                    points
                        ...
                        .attr("cy", function(d) { return y(d.parent.points[d.index].y); })
                        .attr("id", function (d) { return "vertex-" + d.index; })
                        .attr("r", 4/scale)
                    ;
    ...
    // 更改AP设备的颜色，传AP id和颜色，即可更改颜色
    // 颜色支持16进制，颜色单词，rgb
    handleChangeMonitorColor(id, color) {
        d3.select("#vertex-" + id).attr("stroke", color);
    }
                            ...
                            >
                                ...
                                {this.props.guests.map((data, index) => (
                                    <TableRow key={index} selected={data.selected}>
                                        ...
                                        {/*<TableRowColumn>{data.tracks[1]["timeStamp"]}</TableRowColumn>*/}
